...
| 表名 | 说明 | 关键字段 |
|------|------|----------|
| `college` | 学院 | college_id, name, dean |
| `major` | 专业 | major_id, name, college_id(FK) |
| `class` | 班级 | class_id, name, major_id(FK), enroll_year |
| `student` | 学生 | sno(PK), name, gender, class_id(FK), status, enroll_year |
| `teacher` | 教师 | tno(PK), name, title, college_id(FK) |
| `course` | 课程 | cno(PK), name, credit, type, semester, tno(FK) |
| `sc` | 选课/成绩 | (sno,cno) PK, score, status |
| `account` | 账号 | account_id, username, password_hash(SHA256), role |
| `admin` | 管理员 | admin_id, job_no, name, department |
| `major_transfer` | 转专业申请 | transfer_id, sno, from/to_class_id, status |
| `score_abandon` | 放弃成绩申请 | abandon_id, sno, cno, status |
| `reward_punishment` | 奖惩记录 | rp_id, sno, type, title, rp_date |
| `award_application` | 奖项申请 | app_id, sno, title, file_path, status（审批后触发器自动同步到奖励表） |
| `course_video` | 课程视频 | video_id, course_id(FK), title, file_path, uploaded_by(FK) |
| `score_log` | 成绩日志 | log_id, sno, cno, old/new_score, change_time |
